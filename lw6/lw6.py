...
class PascalLexer:
    ...
    def __init__(self, input_file):
        try:
            ...
            print(f"ќшибка: ‘айл '{input_file}' не найден.")
            self.input_file = None
            self.eof = True
        except IOError as e:
            print(f"ќшибка при открытии файла '{input_file}': {e}")
            self.input_file = None
            self.eof = True
    ...
    def next_token(self):
        #отправл€ем строки в буфер, убира€ пробелы
        #строчный коммент -> скип
        #блочный без закрыти€ -> bad
        #слово в кавычках -> string, если не закрыта -> bad
        #ищем регул€ркой цифры и провер€ем на корректность числа: точка без цифр после, >16 разр€дов, символ после <> пробел | разделитель -> bad
        #идентификатор > 256 или пуст -> bad
        while not self.eof or self.buffer:
            #берЄм следующую строку при пустом буфере
            if not self.buffer:
                self.read_next_line()
                if self.eof:
                    return None
            ...
            #ќбрабатываем строки с помощью регул€рки и обновл€ем буфер
            if self.buffer.startswith("'"):
            ...
            if match:
                ...
                #проверка на объвление пустого идентификатора
