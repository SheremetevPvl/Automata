...
class PascalLexer:
    ...
    def next_token(self):
        #отправляем строки в буфер, убирая пробелы
        #строчный коммент -> скип
        #блочный без закрытия -> bad
        #слово в кавычках -> string, если не закрыта -> bad
        #ищем регуляркой цифры и проверяем на корректность числа: точка без цифр после, >16 разрядов, символ после <> пробел | разделитель -> bad
        #идентификатор > 256 или пуст -> bad
        while not self.eof or self.buffer:
            #берём следующую строку при пустом буфере
            if not self.buffer:
                self.read_next_line()
                if self.eof:
                    return None
            ...
            #Обрабатываем строки с помощью регулярки и обновляем буфер
            if self.buffer.startswith("'"):
